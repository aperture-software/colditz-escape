...
static int snd_pcm_set_hwparams(int voice, unsigned int frequency, unsigned int nb_channels, snd_pcm_format_t format)
{
    int err, dir;
    ...
    if (err < 0)
    {
        fprintf(stderr, "audio_backend: Broken configuration for playback - No configurations available: %s\n", snd_strerror(err));
        return err;
    }
    ...
    if (err < 0)
    {
        fprintf(stderr, "audio_backend: Resampling setup failed for playback: %s\n", snd_strerror(err));
        return err;
    }
    ...
    if (err < 0)
    {
        fprintf(stderr, "audio_backend: Access type not available for playback: %s\n", snd_strerror(err));
        return err;
    }
    ...
    if (err < 0)
    {
        fprintf(stderr, "audio_backend: Sample format not available for playback: %s\n", snd_strerror(err));
        return err;
    }
    ...
    if (err < 0)
    {
        fprintf(stderr, "audio_backend: Playout of %d channel(s) not available: %s\n", nb_channels, snd_strerror(err));
        return err;
    }
    /* Set the stream rate */
    ...
    if (err < 0)
    {
        fprintf(stderr, "audio_backend: Rate %iHz not available for playback: %s\n", frequency, snd_strerror(err));
        return err;
    }
    if (rrate != frequency)
    {
        fprintf(stderr, "audio_backend: Unable to match rate (requested %iHz, got %iHz)\n", frequency, rrate);
        return -EINVAL;
    }
    /* Set the buffer time */
    ...
    if (err < 0)
    {
        fprintf(stderr, "audio_backend: Unable to set buffer time %u for voice %i: %s\n", buffer_time[voice], voice, snd_strerror(err));
        return err;
    }
    err = snd_pcm_hw_params_get_buffer_size(params, &size);
    if (err < 0)
    {
        fprintf(stderr, "audio_backend: Unable to get buffer size for playback: %s\n", snd_strerror(err));
        return err;
    }
    ...
    if (err < 0)
    {
        fprintf(stderr, "audio_backend: Unable to set period time %u for voice %i: %s\n", period_time[voice], voice, snd_strerror(err));
        return err;
    }
    err = snd_pcm_hw_params_get_period_size(params, &size, &dir);
    if (err < 0)
    {
        fprintf(stderr, "audio_backend: Unable to get period size for playback: %s\n", snd_strerror(err));
        return err;
    }
    period_size[voice] = size;
//  printf("period_size[%d] = %d, buffer_size[%d] = %d\n", voice, (int)size, voice, (int)buffer_size[voice]);
    /* Write the parameters to device */
    err = snd_pcm_hw_params(handle[voice], params);
    if (err < 0)
    {
        fprintf(stderr, "audio_backend: Unable to set hw params for voice %i: %s\n", voice, snd_strerror(err));
        return err;
    }
    return 0;
}
...
static int snd_pcm_set_swparams(int voice)
{
    ...
    if (err < 0) {
        fprintf(stderr, "audio_backend: Unable to determine current swparams for playback: %s\n", snd_strerror(err));
        return err;
    }
    ...
    if (err < 0) {
        fprintf(stderr, "audio_backend: Unable to set start threshold mode for voice %i: %s\n", voice, snd_strerror(err));
        return err;
    }
    /* Allow the transfer when at least period_size samples can be processed */
    err = snd_pcm_sw_params_set_avail_min(handle[voice], params, period_size[voice]);
    if (err < 0) {
        fprintf(stderr, "audio_backend: Unable to set avail min for voice %i: %s\n", voice, snd_strerror(err));
        return err;
    }
    /* write the parameters to the playback device */
    err = snd_pcm_sw_params(handle[voice], params);
    if (err < 0) {
        fprintf(stderr, "audio_backend: Unable to set sw params for voice %i: %s\n", voice, snd_strerror(err));
        return err;
    }
    return 0;
}
...
bool audio_backend_init(void)
{
    ...
    if ((err = snd_ctl_open(&ctl_pcm, alsa_name, 0)) < 0)
        fprintf(stderr, "audio_backend_init: Could not open sound control: %s\n", snd_strerror(err));
    else if ((err = snd_ctl_card_info(ctl_pcm, info)) < 0)
    {
        printf("audio_backend_init: Could not obtain sound control info: %s\n", snd_strerror(err));
    ...
    for (i=0; i<NB_VOICES; i++)
    {
        ...
        {
            fprintf(stderr, "audio_backend_init: Could not open PCM channel %d: %s\n", i, snd_strerror(err));
            return false;
        }
        if ((err = snd_pcm_nonblock(handle[i], 0)) <0)
        {
            fprintf(stderr, "audio_backend_init: Could not set non-blocking mode: %s\n", snd_strerror(err));
            return false;
        }
    }
    return true;
                             ...
                             unsigned int bits_per_sample, bool stereo)
{
    ...
    case 16:
        ...
        break;
    default:
        fprintf(stderr, "audio_backend_set_voice: Only 8 and 16 bits samples are supported\n");
        return false;
    }
    ...
    for (i = 0; i<(snd_pcm_uframes_t)size; i += period_size[voice])
    {
        if ((err = snd_pcm_writei(handle[voice], (const void*)((uintptr_t)data + i),
            ...
            fprintf(stderr, "audio_backend_set_voice: Could not write to PCM device. %s\n", snd_strerror(err));
            return false;
        }
    }
 ...
 * shoving poll() implementation into a thread...
 */
static void* audio_backend_poll_thread(void *arg)
{
    ...
    if (data->fd < 0)
    {
        fprintf(stderr, "audio_backend_poll_thread: Unable to create event fd: %s\n", strerror(errno));
        goto out;
    }

    count = snd_pcm_poll_descriptors_count(handle[data->voice]);
    if (count <= 0) {
        printf("audio_backend_poll_thread: Invalid poll descriptors count\n");
        goto out;
    }
    ufds = malloc(sizeof(struct pollfd) * count + 1);
    if (ufds == NULL) {
        fprintf(stderr, "audio_backend_poll_thread: Could not allocate ufds!\n");
        goto out;
    }
    err = snd_pcm_poll_descriptors(handle[data->voice], ufds, count);
    if (err < 0) {
        fprintf(stderr, "audio_backend_poll_thread: Unable to obtain poll descriptors for playback: %s\n", snd_strerror(err));
        goto out;
    }
    ...
    while (1)
    {
        while (avail > 0)
        {
            ...
            if (err < 0)
            {
                fprintf(stderr, "audio_backend_poll_thread: Write error: %s\n", snd_strerror(err));
                goto out;
            } else if (err != period_size[data->voice])
            {
                fprintf(stderr, "audio_backend_poll_thread: Write error: written %i expected %li\n", err, period_size[data->voice]);
                goto out;
            }
            ...
            data->callback(&snd_pcm_buffer[data->voice][(buffer_nr+1)%2][period*period_size[data->voice]*data->frame_size],
        ...
        if (revents & POLLERR)
        {
            fprintf(stderr, "audio_backend_poll_thread: Poll error\n");
            goto out;
        }
        if (revents & POLLOUT)
        {
            ...
            if (avail < period_size[data->voice]) {
                fprintf(stderr, "audio_backend_poll_thread: Error, avail is smaller than period_size (%d vs %d)\n",
    ...
    unsigned int frequency, unsigned int bits_per_sample, bool stereo)
{
    ...
    case 16:
        ...
        break;
    default:
        fprintf(stderr, "audio_backend_set_voice_callback: Only 8 and 16 bits samples are supported\n");
        return false;
    }
    ...
    if (buffer == NULL)
    {
        fprintf(stderr, "audio_backend_set_voice_callback: Could not allocate buffers!\n");
        return false;
    }
...
bool audio_backend_start_voice(int voice)
{
    ...
    {
        fprintf(stderr, "audio_backend_start_voice: Voice %d is not in prepared state\n", voice);
        return false;
    }
    if (voice_is_callback[voice])
    {
        /* Create the polling thread */
        err = pthread_create(&snd_pcm_poll_thread_id[voice], NULL, &audio_backend_poll_thread, (void*)&snd_pcm_poll_thread_data[voice]);
        if (err != 0)
        {
            fprintf(stderr, "audio_backend_start_voice: Cannot create thread for voice %i: %s\n", voice, strerror(err));
            return false;
        }
    }
...
bool audio_backend_stop_voice(int voice)
{
    ...
    if (voice_is_callback[voice])
    {
        ...
        {
            fprintf(stderr, "audio_backend_stop_voice: Can not kill thread for voice %d\n", voice);
        }
        else
        {
