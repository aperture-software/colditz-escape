 ...
 *	assign a variable to a table value. Thus the recursive part (_ESCHEW_REC) will
 *	be defined before the macros are set to be used (see further down)
 */
#define XML_CAST_TO_BOOLEAN(tab, idx, val, typ)									\
_ESCHEW_REC(tab, idx, val, typ, xml_boolean,									\
 fprintf(stderr, "illegal cast to boolean\n"))

#define XML_CAST_TO_STRING(tab, idx, val, typ)									\
_ESCHEW_REC(tab, idx, val, typ, xml_string,										\
 fprintf(stderr, "illegal cast to string\n"))

#define XML_CAST_TO_BYTE(tab, idx, val, typ)									\
_ESCHEW_REC(tab, idx, val, typ, xml_unsigned_char,								\
 _ESCHEW_REC(tab, idx, val, typ, xml_char,										\
  fprintf(stderr, "illegal cast to byte"))										\
)

// Parentheses - always a great fun (See also: "Reasons why NOBODY uses Lisp")
#define XML_CAST_TO_INTEGER(tab, idx, val, typ)									\
_ESCHEW_REC(tab, idx, val, typ, xml_unsigned_short,								\
 _ESCHEW_REC(tab, idx, val, typ, xml_short,										\
  _ESCHEW_REC(tab, idx, val, typ, xml_unsigned_int,								\
   _ESCHEW_REC(tab, idx, val, typ, xml_int,										\
    _ESCHEW_REC(tab, idx, val, typ, xml_unsigned_long,							\
     _ESCHEW_REC(tab, idx, val, typ, xml_long,									\
...
_ESCHEW_REC(tab, idx, val, typ, xml_float,										\
 _ESCHEW_REC(tab, idx, val, typ, xml_double,									\
  fprintf(stderr, "illegal cast to floating point\n"))							\
)
...
// These variables are used to free allocated internal resources
static unsigned int _ep_index = 0;
static void** _ep_tbl = NULL;
...
/*
 * Keep track of allocated pointers that need to be released
 */
void to_release(void* allocated)
{
	...
	{
		tbl_tmp = malloc(tbl_size * 2 * sizeof(void*));
		if (tbl_tmp == NULL)
		{
		...
		{
			memcpy(tbl_tmp, _ep_tbl, tbl_size);
			free(_ep_tbl);
			_ep_tbl = tbl_tmp;
		}
	}

	_ep_tbl[_ep_index++] = allocated;
}
...
#undef	_ESCHEW_REC
#endif
#define _ESCHEW_REC(tab, idx, val, typ, def, rem)										\
...
static void XMLCALL assign(void *userData, const char *name)
{
	...
	if (inf->stack[inf->depth].comment != NULL)
	{
		...
		if ((i != -1) && (STACK_NODE(inf->depth-1)->comment[i] == NULL)) {
			...
			to_release(inf->stack[inf->depth].comment);
		}
		else
		{	// If the comment wasn't assigned, we need to free its strdup'd string
	...
	{	// We have a regular value
		...
		case t_xml_string:
			...
			to_release(((xml_string*)STACK_NODE(inf->depth-1)->value)[i]);
			break;
