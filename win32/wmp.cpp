// wmp.cpp: Windows directShow Movie Player
// 
////////////////////////////////////////////////////////////////////////////
...
bool wmp_init(char* app_name)
{
	...
	if (hr != S_OK)
	{
		printf("wmp_init: CoCreateInstance error %X\n", hr);
		wmp_release();
		return false;
	}
	...
	if (hr != S_OK)
	{
		printf("wmp_init: pGB->QueryInterface IID_IMediaControl error %X\n", hr);
		wmp_release();
		return false;
	}
	...
	if (hr != S_OK)
	{
		printf("wmp_init: pGB->QueryInterface IID_IMediaEventEx error %X\n", hr);
		wmp_release();
		return false;
	}
...
bool wmp_play(char* s)
{
	...
	if (FAILED(hr)) 
	{
        printf("Failed(0x%08lx) in RenderFile(%s)!\n", hr, s);
        return false;
    }
    ...
    if (FAILED(hr)) 
	{
        printf("Failed(0x%08lx) in put_Owner!\n", hr, s);
        return false;
    }
